...
public class TradingPlayers
{
    ...
    public TradingPlayers() 
    {
        ...
        ModLoadingContext.get().registerConfig(Type.SERVER, TradingPlayersConfig.SPEC);
    ...
    @SubscribeEvent
    public void playerDisconnect(final PlayerEvent.PlayerLoggedOutEvent event)
    {
        DistExecutor.safeRunWhenOn(Dist.DEDICATED_SERVER, () -> {
            return new DistExecutor.SafeRunnable() {
                public void run() {
                    Session.getPlayerSession((ServerPlayer) event.getEntity()).ifPresent(session -> {
    ...
    public static void openTradingScreen(ServerPlayer p)
    {
    	MenuProvider provider = new MenuProvider() 
    	{
			...
			{
				return Component.translatable("trade.name");
			}
    	};
