...
public class Session implements Container
{
	...
	public static void toggleAcceptanceFor(@Nonnull ServerPlayer sender) 
	{
		Session.getPlayerSession(sender).ifPresent(s -> {
			if(sender == s.player)
			{
				...
				PacketHandler.INSTANCE.send(new PacketUpdateTradingStateClient((byte) 0), PacketDistributor.PLAYER.with(s.trader));
			}
			else
			{
				s.traderAcceptance = !s.traderAcceptance;
				PacketHandler.INSTANCE.send(new PacketUpdateTradingStateClient((byte) 0), PacketDistributor.PLAYER.with(s.player));
			}
	...
	public static void resetBothAcceptances(ServerPlayer sender)
	{
		Session.getPlayerSession(sender).ifPresent(s -> {
			...
			PacketHandler.INSTANCE.send(new PacketUpdateTradingStateClient((byte) 1), PacketDistributor.PLAYER.with(s.player));
			PacketHandler.INSTANCE.send(new PacketUpdateTradingStateClient((byte) 1), PacketDistributor.PLAYER.with(s.trader));
		});
	}
