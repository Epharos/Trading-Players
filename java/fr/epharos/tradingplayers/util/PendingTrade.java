		...
		return new DistExecutor.SafeRunnable() 
		{
			...
			public void run() 
			{
				for(Iterator<PendingTrade> it = PendingTrade.pendingTrades.iterator() ; it.hasNext() ;)
				{
					...
					if(pt.expiry <= System.currentTimeMillis())
					{
						pt.player.sendMessage(new TranslatableComponent("chat.trade.noanswer.player", pt.trader.getDisplayName().getString()), Util.NIL_UUID);
						pt.trader.sendMessage(new TranslatableComponent("chat.trade.noanswer.trader", pt.player.getDisplayName().getString()), Util.NIL_UUID);
						it.remove();
					}
				}
			}
