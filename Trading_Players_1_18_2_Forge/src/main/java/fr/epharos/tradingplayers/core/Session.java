...
public class Session implements Container
{
	...
	private void destroySession() 
	{
		...
		{
			player.sendMessage(new TranslatableComponent("chat.trade.success", trader.getName().getString()), Util.NIL_UUID);
			trader.sendMessage(new TranslatableComponent("chat.trade.success", player.getName().getString()), Util.NIL_UUID);
			TradingPlayers._LOGGER.info("Trade processed (" + player.getName().getString() + " <-> " + trader.getName().getString() + ")");
		}
		else
		{
			player.sendMessage(new TranslatableComponent("chat.trade.cancelled", trader.getName().getString()), Util.NIL_UUID);
			trader.sendMessage(new TranslatableComponent("chat.trade.cancelled", player.getName().getString()), Util.NIL_UUID);
