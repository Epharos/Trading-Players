...
public class PacketTraderName 
{
	...
	public static boolean handlePacket(PacketTraderName packet, Supplier<NetworkEvent.Context> ctx)
	{
		final AtomicBoolean flag = new AtomicBoolean(false);

		if(Minecraft.getInstance().screen instanceof TradingScreen s)
		{
			s.traderHead = ((AbstractClientPlayer)(Minecraft.getInstance().level.getPlayerByUUID(packet.playerID))).getSkinTextureLocation();
		}
